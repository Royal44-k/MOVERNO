...
check('HTML lists all 15 concepts',[...html.matchAll(/<article id="shoe-/g)].length===15);
check('metadata says 15 completed',data.completed===15);
const layout=JSON.parse(await fs.readFile(base+'/layout-qa-batch02-03.json','utf8'));
for(const p of selected.slice(5)){
 const m=await sharp(base+'/'+p.atlas).metadata();
 check(p.id+' six explicit cell boxes',layout.cells[p.id]?.length===6);
 check(p.id+' cells inside image',layout.cells[p.id]?.every(([x,y,w,h])=>x>=0&&y>=0&&w>0&&h>0&&x+w<=m.width&&y+h<=m.height));
 check(p.id+' two brand closeups',layout.brandCrops[p.id]?.length===2);
}
for(const b of ['02','03']){
 const batchHtml=await fs.readFile(base+'/index-batch'+b+'.html','utf8');
 check('batch '+b+' has 5 cards',[...batchHtml.matchAll(/<article id="shoe-/g)].length===5);
}
const result={date:'2026-09-09',scope:'all 15 / batches 01–03',count:checks.length,passed:checks.filter(c=>c.ok).length,failed:checks.filter(c=>!c.ok),checks,visualReview:'Source silhouettes and palette, six-view order, cover composition and detail crops inspected separately. Added views are conceptual, not CAD/tech-pack or factory proof.'};
await fs.writeFile(base+'/verification-complete.json',JSON.stringify(result,null,2));
